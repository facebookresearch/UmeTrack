...
if __name__ == "__main__":
    dataset_names = ["cropped", "manos_cropped", "hhi_cropped", "hhi_manos_cropped"]
    dataset_names = ["hhi_cropped"]

    device: str = "cuda" if torch.cuda.device_count() else "cpu"
    print(f"Using device: {device}")
    datasets_all = [os.path.join(root, "torch_data", s) for s in dataset_names]

    fields = ["mono", "labels"]
    datasets = find_dataset(
        datasets_all,
        fields,
        # cache_in_mem_fields=["labels"],
    )
    print("Dataset stats")
    ...
    model_path = os.path.join(
        root, "DPE_release", "pretrained_models", "f344158181.torch"
    )
    ...
    # run_splits = [Split.TRAIN, SPlit.TEST]
    run_splits = [Split.TRAIN]
