...
if __name__ == '__main__':
    ...
    error_tensors = []
    # Hack to use a subset first
    is_test_run = False
    if not is_test_run:
        input_dir = os.path.join(root, "raw_data", "real")
        output_dir = os.path.join(root, "eval_results_known_skel_0224_1605", model_name, "real")
        ...
        output_dir = os.path.join(root, "eval_results_test", model_name, "real")
        input_paths, output_paths = _find_input_output_files(input_dir, output_dir, test_only=True)
        for input_output in zip(input_paths, output_paths):
            error_tensors.append(_track_sequence(input_output, model_path=model_path, override=True))
