...
class Sampler(torch.utils.data.Sampler[int]):
    ...
    distrib_info
        Sometimes people may want to construct the sampler outside of s2. In
        this situation, passing in distrib_info=(1, 1) is an easy work around.

        Note that you should only specify this if you want to bypass s2.distrib.
